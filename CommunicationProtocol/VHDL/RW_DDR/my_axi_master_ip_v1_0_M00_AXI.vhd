...
entity my_axi_master_ip_v1_0_M00_AXI is
	generic (
...
end my_axi_master_ip_v1_0_M00_AXI;

architecture implementation of my_axi_master_ip_v1_0_M00_AXI is
	...
	signal w_t_1:std_logic;
	signal w_t_2:std_logic;
	signal w_tx:std_logic;
	signal inter_w_state_tx:std_logic:='0';
	signal inter_w_state_done:std_logic:='0';
	------> read pulse signal
	signal r_pulse:std_logic;
	signal r_t_1:std_logic;
	signal r_t_2:std_logic;
	signal r_tx: std_logic;
	signal inter_r_state_tx:std_logic:='0';
    signal inter_r_state_done:std_logic:='0';
...
begin
    ...
    ---- W pulse generator
     w_pulse<= (not w_t_2) and w_t_1;
     w_tx<=USER_WRITE_TX;
     inter_w_state_tx<=w_tx;
     USER_W_STATE_TX<=inter_w_state_tx;
     process(M_AXI_ACLK)                                                          
        begin                                                                             
    ...
    ---- R pulse generator
    r_pulse<= (not r_t_2) and r_t_1;
    r_tx<=USER_READ_TX;
    inter_r_state_tx<=r_tx;
    USER_R_STATE_TX<=inter_r_state_tx;
         process(M_AXI_ACLK)                                                          
            begin                                                                             
              if (rising_edge (M_AXI_ACLK)) then                                              
                  -- Initiates AXI transaction delay        
	...
	-- request on the same clock cycle. 
	  ...
	  begin                                                                             
	    if (rising_edge (M_AXI_ACLK)) then                                              
	     ...
	     ----> User Modify
	         if (M_AXI_ARESETN = '0' or w_pulse = '1') then
	     ----> End Modify     
	     ----> Original:  if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
	        axi_awvalid <= '0';                                                         
	      else                                                                          
	  ...
	  begin                                                                             
	    if (rising_edge (M_AXI_ACLK)) then   
	     ----> User  Modify
	      if (M_AXI_ARESETN = '0' or w_pulse = '1') then  
	     ----> End Modify                                           
	     ----> Origianl: if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
	...
	--keep track with number of read transaction issued/initiated
	  ...
	  begin                                                                            
	    if (rising_edge (M_AXI_ACLK)) then
	    ----> User Modify
	     if (M_AXI_ARESETN = '0' or r_pulse = '1') then 
	    ----> End Modify                                             
	    ----> Original: if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                               
	        read_index <= (others => '0');                                             
	      else                                                                         
	    ...
	    process(M_AXI_ACLK)                                                                 
	      begin                                                                            
	    	...
	    	----> Original:  if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
	    	    ...
	    	    axi_awaddr <= USER_W_ADDR;  
	    	    ----> End Modify                                                  
	    	    ----> Original: axi_awaddr <= std_logic_vector (unsigned(axi_awaddr) + 4);                                     
	    	  end if;                                                                       
	    	end if;                                                                         
	      end process;                                                                     
	   	  ...
	   	  begin                                                                         
	   	    ...
	   	    ----> End Modify                                       
	   	     ----> Original: if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' ) then                                            
	   	        ...
	   	        -- available by user logic 
	   	        ----> User Modify
	   	        axi_araddr <= USER_R_ADDR;
	   	        ----> End Modify                                             
	   	        ----> Original: axi_araddr <= std_logic_vector (unsigned(axi_araddr) + 4);                                 
	   	      end if;                                                                   
	   	    end if;                                                                     
	   	  end process;                                                                  
	    ...
	    process(M_AXI_ACLK)                                                                
		  begin                                                                             
		    if (rising_edge (M_AXI_ACLK)) then 
		     ...
		     ---->End Modify                                     
		     ----> Original:  if (M_AXI_ARESETN = '0' or init_txn_pulse = '1') then                                                
		     ---->  Original: axi_wdata <= C_M_START_DATA_VALUE;    	                                    
		      elsif (M_AXI_WREADY = '1' and axi_wvalid = '1') then                          
		        -- Signals a new write address/ write data is                               
		        -- available by user logic  
		        ----> User Modify
		        axi_wdata <=USER_W_DATA;
		        ----> End Modify                                                
		        ----> Original: axi_wdata <= std_logic_vector (unsigned(C_M_START_DATA_VALUE) + unsigned(write_index));    
		      end if;                                                                       
		    end if;                                                                         
		  end process;                                                                      
	...
	-- Expected read data  
	    ...
	    begin                                                                              
	      if (rising_edge (M_AXI_ACLK)) then     
	        ...
	        ----> End Modify                             
	        ---->Original: if (M_AXI_ARESETN = '0' or init_txn_pulse = '1' ) then                    
	        ---->Original:   expected_rdata <= C_M_START_DATA_VALUE;    	                                
	          ...
	          ----> End Modify                                   
	          ----> Original:  expected_rdata <= std_logic_vector (unsigned(C_M_START_DATA_VALUE) + unsigned(read_index)); 
	        end if;                                                                        
	      end if;                                                                          
	    end process;                                                                       
	...
	-- */    
	---->User Modify
	USER_W_STATE_DONE<=inter_w_state_done;
	inter_w_state_done<=writes_done;
	USER_W_DONE<=writes_done;
	---->End Modify                                                                                           
	  process(M_AXI_ACLK)                                                                               
	  begin                                                                                             
	    if (rising_edge (M_AXI_ACLK)) then   
	...
	---->User Modify
        ...
        USER_R_STATE_DONE<=inter_r_state_done;
        inter_r_state_done<=reads_done;
        USER_R_DONE<=reads_done;
